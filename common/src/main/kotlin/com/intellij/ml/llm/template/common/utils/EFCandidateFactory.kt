package com.intellij.ml.llm.template.common.utils
...
class EFCandidateFactory {
    ...
    private fun adjustIfBlockHeuristic(candidate: EFCandidate, file: PsiFile): EFCandidate {
        ...
        if (firstStatement is PsiIfStatement && PsiTreeUtil.isAncestor(firstStatement, lastStatement, true)) {
            val idx = statements.indexOfFirst { it !is PsiBlockStatement && it !is PsiIfStatement}
            if (idx != -1) {
                val newCandidate = EFCandidate(
                    functionName = candidate.functionName,
                    offsetStart = statements[idx].startOffset,
                    offsetEnd = lastStatement.startOffset,
                    lineStart = statements[idx].getLineNumber(true) + 1,
                    lineEnd = lastStatement.getLineNumber(false) + 1
                ).also {
