...
import com.intellij.ml.llm.template.common.models.ExtractFunctionLLMRequestProvider
...
class UtilsTest : LightPlatformCodeInsightTestCase() {
    ...
    fun `test count lines of code of a method in Java`() {
        configureByFile("/testdata/KafkaAdminClientTest.java")
        val offset = 8314
        val method = PsiUtils.getParentFunctionOrNull(offset, file)
        val expectedLoc = 5

        TestCase.assertEquals(expectedLoc, PsiUtils.countLinesOfCode(method!! as PsiMethod))
    }

    fun `test count lines of code of a method in Java 2`() {
        configureByFile("/testdata/KafkaAdminClientTest.java")
        val offset = 8127
        val method = PsiUtils.getParentFunctionOrNull(offset, file)
        val expectedLoc = 2
    ...
    fun `test count lines of code of a method in Java 3`() {
        configureByFile("/testdata/KafkaAdminClientTest.java")
        val offset = 8384
        val method = PsiUtils.getParentFunctionOrNull(offset, file)
        val expectedLoc = 4
    ...
    fun `test duplicates are correctly removed from list of EFCandidate`() {
        val candidates = listOf(
            EFCandidate("foo", 0, 0, 1, 1),
            EFCandidate("foo", 0, 0, 1, 1),
            EFCandidate("foo", 0, 0, 1, 2),
            EFCandidate("foo", 0, 0, 1, 1),
            EFCandidate("foo", 0, 0, 1, 2),
            EFCandidate("foo", 0, 0, 2, 5),
        )
        val expected = listOf(
            EFCandidate("foo", 0, 0, 1, 1),
            EFCandidate("foo", 0, 0, 1, 2),
        ...
        val actual = removeDuplicates(candidates)
