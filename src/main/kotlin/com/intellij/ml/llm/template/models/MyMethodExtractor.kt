...
class MyMethodExtractor(private val functionNameProvider: FunctionNameProvider? = null) {
    fun doExtract(file: PsiFile, range: TextRange, extractFunctionType: MethodExtractionType) {
        ...
        if (EditorSettingsExternalizable.getInstance().isVariableInplaceRenameEnabled) {
            ...
            if (activeExtractor != null) {
                ...
                findAndSelectExtractOption(editor, file, range, extractFunctionType)?.thenApply { options ->
    ...
    fun findAndSelectExtractOption(
        editor: Editor,
        file: PsiFile,
        range: TextRange,
        extractFunctionType: MethodExtractionType
    ): CompletableFuture<ExtractOptions>? {
        try {
    ...
    private fun <T, E : Exception> computeWithAnalyzeProgress(
        project: Project,
        throwableComputable: ThrowableComputable<T, E>
    ): T {
        return ProgressManager.getInstance().run(object : Task.WithResult<T, E>(
            project,
    ...
    private fun suggestSafeMethodNames(options: ExtractOptions): List<String> {
        ...
        val generatedNames =
            sequenceOf(baseName) + generateSequence(1) { seed -> seed + 1 }.map { number -> "$baseName$number" }
    ...
    private fun createInplaceSettingsPopup(options: ExtractOptions): ExtractMethodPopupProvider {
        ...
        val hasAnnotation =
            options.dataOutput.nullability != Nullability.UNKNOWN && options.dataOutput.type !is PsiPrimitiveType
    ...
    fun extractMethod(extractOptions: ExtractOptions): ExtractedElements {
        ...
        return replaceElements(
            extractOptions.elements,
            elementsToExtract.callElements,
            extractOptions.anchor,
            elementsToExtract.method
        )
    }
    ...
    fun doTestExtract(
        ...
        var options = allOptionsToExtract.takeIf { targetClass != null }
            ?.find { option -> option.anchor.containingClass == targetClass }
        ...
        if (isConstructor != options.isConstructor) {
            options = ExtractMethodPipeline.asConstructor(analyzer, options) ?: throw ExtractException(
                "Fail",
                elements.first()
            )
        }
        if (!options.isStatic && isStatic == true) {
            options = ExtractMethodPipeline.withForcedStatic(analyzer, options) ?: throw ExtractException(
                "Fail",
                elements.first()
            )
        }
        if (newNameOfFirstParam != null) {
            options = options.copy(
                inputParameters = listOf(
                    options.inputParameters.first().copy(name = newNameOfFirstParam)
                ) + options.inputParameters.drop(1)
            )
        }
        if (returnType != null) {
    ...
    fun showError(editor: Editor, ranges: List<TextRange>) {
        ...
        WindowManager.getInstance().getStatusBar(project).info =
            RefactoringBundle.message("press.escape.to.remove.the.highlighting")
    }
...
//            updateMethodAnnotations(method, dependencies.inputParameters)
    ...
    private fun needsNullabilityAnnotations(project: Project): Boolean {
        return PropertiesComponent.getInstance(project)
            .getBoolean(ExtractMethodDialog.EXTRACT_METHOD_GENERATE_ANNOTATIONS, true)
    }
    ...
    companion object {
        ...
        @NonNls
        const val refactoringId: String = "refactoring.extract.method"
        ...
        fun invokeOnElements(
            ...
            if (selection != null) MyMethodExtractor(functionNameProvider).doExtract(
                file!!,
                selection,
                extractFunctionType
            )
