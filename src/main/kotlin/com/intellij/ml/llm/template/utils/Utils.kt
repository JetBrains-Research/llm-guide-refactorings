...
private fun isFunctionExtractableJava(
    ...
    if (!efCandidate.isValid()) {
        ...
        val elements = if (range == null) emptyList() else ExtractSelector().suggestElementsToExtract(file, range)
        if (elements.isEmpty()) {
            result = false
            applicationResult = EFApplicationResult.FAIL
...
class PsiUtils {
    companion object {
        ...
        fun getTopLevelStatementsBetweenOffsets(startOffset: Int, endOffset: Int, psiFile: PsiFile): List<PsiStatement> {
            // Get PsiElements at the start and end offsets
            val startElement = psiFile.findElementAt(startOffset) ?: return emptyList()
            val endElement = psiFile.findElementAt(endOffset) ?: return emptyList()
            ...
            // Extract all PsiStatements within the common parent
            val allStatementsInParent = PsiTreeUtil.getChildrenOfType(commonParent, PsiStatement::class.java)
                ?: return emptyList()
