...
class UtilsTest: BasePlatformTestCase() {
    fun `test identify extract function suggestions from chatgpt reply`() {
        ...
        TestCase.assertEquals(2650 to 2656, efSuggestionList.suggestion_list.get(0).lineStart to efSuggestionList.suggestion_list.get(0).lineEnd)
        TestCase.assertEquals("createConsumerCoordinator", efSuggestionList.suggestion_list.get(1).functionName)
        TestCase.assertEquals(2649 to 2670, efSuggestionList.suggestion_list.get(1).lineStart to efSuggestionList.suggestion_list.get(1).lineEnd)
        TestCase.assertEquals("createFetchConfig", efSuggestionList.suggestion_list.get(2).functionName)
        TestCase.assertEquals(2674 to 2684, efSuggestionList.suggestion_list.get(2).lineStart to efSuggestionList.suggestion_list.get(2).lineEnd)
        TestCase.assertEquals("createFetcher", efSuggestionList.suggestion_list.get(3).functionName)
        TestCase.assertEquals(2685 to 2692, efSuggestionList.suggestion_list.get(3).lineStart to efSuggestionList.suggestion_list.get(3).lineEnd)
        TestCase.assertEquals("createOffsetFetcher", efSuggestionList.suggestion_list.get(4).functionName)
        TestCase.assertEquals(2693 to 2701, efSuggestionList.suggestion_list.get(4).lineStart to efSuggestionList.suggestion_list.get(4).lineEnd)
        TestCase.assertEquals("createTopicMetadataFetcher", efSuggestionList.suggestion_list.get(5).functionName)
        TestCase.assertEquals(2702 to 2703, efSuggestionList.suggestion_list.get(5).lineStart to efSuggestionList.suggestion_list.get(5).lineEnd)
    }
    ...
    fun `test replace github url line range`() {
        val url="https://github.com/apache/kafka/blob/trunk/clients/src/test/java/org/apache/kafka/common/requests/UpdateMetadataRequestTest.java#L81-L212"
        val expectedUrl = "https://github.com/apache/kafka/blob/trunk/clients/src/test/java/org/apache/kafka/common/requests/UpdateMetadataRequestTest.java#L90-L120"
